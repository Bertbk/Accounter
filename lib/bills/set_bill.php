...
function set_bill($account_id_arg, $title_bill_arg, $description_arg="")
{
	...
	$does_this_guy_exists = get_bill_by_name($account_id, $name_of_bill);
	if(!empty($does_this_guy_exists))
	{
?>
<script type="text/javascript">
  alert('Bill with the same name already reccorded!');
</script>
<?php
		return false;
	}
	...
	try
	{
		...
		VALUES(NULL, :account_id, :hashid, :title, :description)';
		$prepare_query = $db->prepare($myquery);
		$prepare_query->bindValue(':account_id', $account_id, PDO::PARAM_INT);
		$prepare_query->bindValue(':hashid', $hashid, PDO::PARAM_STR);
		$prepare_query->bindValue(':title', $title, PDO::PARAM_STR);
