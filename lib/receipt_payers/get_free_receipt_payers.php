...
include_once(LIBPATH.'/receipt_payers/is_this_participant_in_receipt.php');
...
function get_free_receipt_payers($account_id_arg)
{
	...
	foreach($my_participants as $participant)
	{
		...
		{
			$is_in_this_receipt = is_this_participant_in_receipt($account_id, $receipt['id'], $participant['id']);
