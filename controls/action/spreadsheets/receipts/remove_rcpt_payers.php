...
if(isset($_POST['submit_remove_all_rcpt_payers']))
{
	...
	if(empty($errArray))
	{
		$rcpt_payers = get_rcpt_payers_by_spreadsheet_id($account['id'], $spreadsheet['id']);
		//Delete the participants
		foreach($rcpt_payers as $rcpt_payer)
		{
			$success = delete_rcpt_payer($account['id'], $rcpt_payer['id']);	
			if($success === true)
				{	array_push($successArray, 'Payers has been successfully deleted');}
			else
