...
include_once(LIBPATH.'/spreadsheets/receipts/rcpt_recipients/get_rcpt_recipients_by_spreadsheet_id.php');
...
if(isset($_POST['submit_remove_all_rcpt_recipients']))
{
	...
	if(empty($errArray))
	{
		$rcpt_recipients = get_rcpt_recipients_by_spreadsheet_id($account['id'], $spreadsheet['id']);
		//Delete the participants
		foreach($rcpt_recipients as $rcpt_recipient)
		{
			$success = delete_rcpt_recipient($account['id'], $rcpt_recipient['id']);	
			if($success === true)
				{	array_push($successArray, 'recipients has been successfully deleted');}
			else
