...
if(isset($_POST['submit_new_participant']))
{
	...
	if(!is_null($result))
	{
		array_push($errArray, 'No input data');
	}
	...
	if(empty($_POST[$key])) { //If empty
		...
		$result[$key] = filter_input(INPUT_POST, $key, FILTER_SANITIZE_STRING);
		if($result[$key] === false)
	...
	if(empty($_POST[$key])) { //If empty
		...
		$result[$key] = filter_input(INPUT_POST, $key, FILTER_SANITIZE_NUMBER_INT);
	...
	if(!empty($_POST[$key]))
	{
		...
		$result[$key] = filter_var($result[$key], FILTER_SANITIZE_EMAIL);
		if($result[$key] === false)
		{array_push($errArray, $ErrorMessage[$key]);}
	}
	...
	if((empty($errArray))
	{
		set_participant($account_id_arg, $hash_id_arg, $name_of_participant_arg, $nb_of_people_arg, $email_arg)
